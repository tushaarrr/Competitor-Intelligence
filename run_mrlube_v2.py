"""Run Mr. Lube + Tires v2 scraper (Phase 5).

Reads ``app/config/competitors.v2.json`` and only scrapes Mr. Lube + Tires.
Other competitors and the legacy pipeline are NOT affected.

Usage:
    python run_mrlube_v2.py                  # default (--qa-expanded)
    python run_mrlube_v2.py --final-deduped  # collapse fan-out duplicates
    python run_mrlube_v2.py --no-ocr         # text-only run (skip image OCR)
"""
import argparse
import csv
import json
...
from app.scrapers.mrlube_scraper import scrape_mrlube_v2  # noqa: E402
...
def find_mrlube(config_path: Path) -> dict:
    data = json.loads(config_path.read_text())
    for entry in data:
        if entry.get("competitor", "").strip().lower() == "mr. lube + tires":
            return entry
    raise SystemExit("Mr. Lube + Tires not found in competitors.v2.json")
...
def export_url_coverage(result: dict, dest: Path, competitor: str) -> Path:
    ...
    fields = [
        "competitor", "url", "scope", "service_hint", "status",
        "cards_on_page", "text_extracted_count", "image_ocr_extracted_count",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Mr. Lube + Tires v2 scraper")
    g = parser.add_mutually_exclusive_group()
    g.add_argument("--qa-expanded", dest="mode", action="store_const",
                   const="qa_expanded",
                   help="QA mode (default): keep every fan-out row.")
    g.add_argument("--final-deduped", dest="mode", action="store_const",
                   const="final_deduped",
                   help="Collapse fan-out duplicates per city for client delivery.")
    parser.add_argument("--no-ocr", action="store_true",
                        help="Skip image OCR; text-only run.")
    ...
    if not cfg.exists():
        ...
        return 1
    entry = find_mrlube(cfg)

    print("=" * 70)
    print("Mr. Lube + Tires v2 - Phase 5 run")
    print("=" * 70)
    ...
    t0 = time.time()
    result = scrape_mrlube_v2(entry, mode=args.mode, enable_ocr=not args.no_ocr)
    ...
    print(f"  unique_promo_descriptions  : {len(val.get('unique_promo_descriptions') or [])}")
    print(
    ...
    if url_log:
        ...
        for e in url_log:
            print(
                f"  [{e.get('service_hint','?'):<14}|{e.get('status','?'):<13}] "
                f"text={e.get('text_extracted_count', 0):<2} "
                f"img_ocr={e.get('image_ocr_extracted_count', 0):<2} "
                f"img_ocr_fail={e.get('image_ocr_failed_needs_review_count', 0):<2} "
    ...
    if promos:
        print(f"Sample rows (first 5 of {len(promos)}):")
        for i, p in enumerate(promos[:5], 1):
            nr = " [needs_review]" if p.get("needs_review") else ""
            print(
                f"  {i}. [{p.get('city'):<10}|{p.get('service_name'):<18}|"
                f"{p.get('extraction_method'):<17}|conf={p.get('confidence')!s:<5}] "
                f"d={p.get('discount_value')!r} c={p.get('coupon_code')!r} "
                f"exp={p.get('expiry_date')!r}{nr}"
            )
            print(f"     title: {p.get('ad_title','')[:90]}")
            print(f"     desc : {p.get('promo_description','')[:90]}")
        print()

    json_path = ROOT / "data" / "promotions" / "mrlube_v2.json"
    csv_path = ROOT / "data" / "promotions" / "mrlube_v2.csv"
    coverage_path = ROOT / "data" / "promotions" / "mrlube_v2_url_coverage.csv"
    excluded_path = ROOT / "data" / "promotions" / "mrlube_v2_excluded_rows.csv"
